...
% Updated: 2016/09/29 /Yao Zhang---Add a(gamma)
% Useage: 
...
if nargin>11%Updated by Yao 02092017
...
X=size(layers);
if X(2)~=6|~isnumeric(layers)% Updated by Yao 09/29/2016
    error('Invalid layers matrix. Must be an Xx6 numerical matrix');
end
...
% fprintf(fid, '\n%s \t\t# Phase Function filename',PhaseFunctionFilename); % Phase Function filename
...
% h.PhaseFunctionFilename=PhaseFunctionFilename;
