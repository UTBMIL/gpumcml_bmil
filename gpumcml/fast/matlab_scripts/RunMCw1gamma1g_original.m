...
%     mua_v  = [0.01,5]; % absorption vector (cm^-1)
    ...
    for mua_e = mua_v
        for gamma = gammas % To run multiple values of gammas we will need to change the exe.file. Please use one gamma each time for now
            for musp_v = musp_vs
                if isfile(['Test/Simulation_gamma' num2str(gamma) '_musp_' num2str(musp_v) '_g_' num2str(g1) '_mua_' num2str(mua_e) '.mat']) || isfile(['Test/SFDR/SFDR_mu_' num2str(musp_v) '_gamma_' num2str(gamma) '_g_' num2str(g1) '_mua_' num2str(mua_e) '.mat'])
                    continue;
                else
                    exist_flag = 0;
                end
